...
export default function UserUploadDataModal({ stepid, onClose, refreshData }) {
    const [dataReqUser, setDataReqUser] = useState("");
    const [dataUserFileUpload, setDataUserFileUpload] = useState(null);
    ...
    const handleFileChange = (e) => {
        setDataUserFileUpload(e.target.files[0]);
    };

    const handleTextChange = (e) => {
        setDataReqUser(e.target.value);
    };

    const clearForm = () => {
        setDataReqUser("");
        setDataUserFileUpload(null);
    };

    const handleSubmit = async () => {
        if (!dataReqUser && !dataUserFileUpload) {
            Swal.fire({
                icon: "warning",
        ...
        if (dataReqUser) formData.append("data_req_user", dataReqUser);
        if (dataUserFileUpload) formData.append("data_user_file_upload", dataUserFileUpload);

        try {
            const response = await axios.patch(
                `${Api_Uri}/services/orders/steps/${stepid}/update`,
                formData,
                {
                    headers: {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" dir="rtl">
            <div className="bg-white rounded-lg shadow-lg w-[40rem] p-6 overflow-y-auto max-h-[70vh]">
                ...
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-semibold text-gray-800">تحميل بيانات المستخدم</h2>
                ...
                <div className="space-y-4">
                    {/* Text Area for Data Request */}
                    <div>
                        <label htmlFor="dataReqUser" className="block text-sm font-medium text-gray-700 mb-2">
                            إدخال البيانات
                        </label>
                        <textarea
                            id="dataReqUser"
                    ...
                    <div>
                        <label htmlFor="dataUserFileUpload" className="block text-sm font-medium text-gray-700 mb-2">
                            تحميل ملف البيانات
                        </label>
                        <input
                            id="dataUserFileUpload"
                            type="file"
                ...
                <div className="mt-6 flex justify-between items-center gap-4 flex-wrap">
                    <button
                        ...
                        className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600  md:w-full lg:w-50"
                        disabled={loading}
                    >
                        {loading ? "جارٍ التحديث..." : "تحميل البيانات"}
                    </button>
                    <button
                        onClick={onClose}
                        className="bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400 md:w-full lg:w-50"
                    >
                        إغلاق
                    </button>
                </div>
