...
import { FaUserAlt, FaEnvelope, FaPhone, FaGlobe, FaBriefcase, FaBuilding } from 'react-icons/fa';
...
export default function Page() {
    ...
    const fetchingData = () => {
        axios
            .get(`${Api_Uri}/auth/employees/full-logged`, {
                headers: {
    ...
    // Handle company data form change
    ...
    // Handle form submission for company data update
    const handleCompanyDataSubmit = (e) => {
        e.preventDefault();

        // Example: You can use this to send the data to an API or update state
        console.log('Updated Company Data:', companyData);

        axios.put(`${Api_Uri}/auth/employees/update/${companyData.employee_id}`,
            {
                "user": companyData.user_id,
            ...
            .then(response => {
                Swal.fire({
                    icon: 'success',
                    title: 'Success',
                    text: 'Company data updated successfully!',
                    timer: 1500,
                    showConfirmButton: false,
                    timerProgressBar: true,
                    didOpen: () => {
                ...
                fetchingData(); // Fetch data again after updating
                handleCloseSmallModal(); // Close modal on success
            })
            .catch(error => {
                Swal.fire({
                    icon: 'error',
    ...
    return (
        <SidebarLayout currentTab="account">
            <div className="min-h-screen flex flex-col" dir="rtl">
                    ...
                    {data ? (
                        <div className="bg-white p-8 rounded-xl shadow-2xl flex flex-col items-center space-y-8">
                            <div className="relative">
                                <img
                                    src={data.user.profile_picture || 'https://placehold.it/100x100'}
                            ...
                            <p className="text-sm text-gray-500 italic">{data.user.role}</p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
                                ...
                                <div className="bg-gray-50 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300">
                                    ...
                                    <p className="text-lg font-semibold text-gray-800">{data.user.date_of_birth}</p>
                                </div>
                                
                                <div className="bg-gray-50 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300">
                                    <FaBriefcase className="text-red-500 text-4xl mb-4" />
                                    <h3 className="text-md font-medium text-gray-600">المنصب</h3>
                                    <p className="text-lg font-semibold text-gray-800">{data.position}</p>
                                </div>

                                <div className="bg-gray-50 p-6 rounded-lg shadow-md hover:shadow-xl transition-all duration-300">
                                    <FaBuilding className="text-purple-500 text-4xl mb-4" />
                                    <h3 className="text-md font-medium text-gray-600">اسم الشركة</h3>
                                    <p className="text-lg font-semibold text-gray-800">{data.company_name}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                                ...
                                <button
                                    ...
                                    className="px-8 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white text-lg font-medium rounded-lg shadow-lg hover:opacity-90 transition-all duration-300"
                                >
                                    تحديث بيانات الشركة
                                </button>
                ...
                {/* Smaller Modal for company data */}
                {isSmallModalOpen && (
                    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center">
                        <div className="bg-white rounded-lg p-6 w-11/12 max-w-md">
                            <h2 className="text-xl font-semibold text-gray-800 mb-4">تحديث بيانات الشركة</h2>
                            <form onSubmit={handleCompanyDataSubmit} className="space-y-4">
                                {/* Position Field */}
                                <div>
                                    <label htmlFor="position" className="block text-gray-600 mb-2">المنصب</label>
                                    <input
                                        type="text"
                                        id="position"
                                        name="position"
                                ...
                                {/* Company Name Field */}
                                <div>
                                    <label htmlFor="company_name" className="block text-gray-600 mb-2">اسم الشركة</label>
                                    <input
                                        type="text"
                                        id="company_name"
                                        name="company_name"
